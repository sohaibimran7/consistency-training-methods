...
Used by scripts/tinker_training/train_rl.py and train_sft.py (and any future
...
def add_backend_args(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument("--backend", default="tinker", choices=["tinker", "local"],
                   help="'tinker' (managed service, default) or 'local' (in-process torch/PEFT "
                        "on this machine's GPUs — the Isambard/Vast.ai path)")
    g.add_argument("--local-device", default=None,
                   help="LocalBackend device (default: cuda if available, else cpu)")
    g.add_argument("--local-dtype", default="bfloat16", choices=list(_DTYPES),
                   help="LocalBackend model dtype")
    g.add_argument("--local-sampler", default="vllm", choices=["vllm", "hf"],
    ...
    g.add_argument("--local-gpu-mem-util", type=float, default=0.45,
                   help="vLLM gpu_memory_utilization — leave headroom for the training model "
                        "when colocated on one GPU")
    g.add_argument("--local-full-finetune", action="store_true",
...
def describe_backend(args: argparse.Namespace) -> str:
    ...
    return (f"local ({args.local_device or 'auto'}, {args.local_dtype}, "
