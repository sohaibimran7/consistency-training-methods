...
The catalog below is therefore explicitly labelled ``reconstruction_v2``.
...
WRAPPER_CATALOG_VERSION = "reconstruction_v2"
...
# Backward-compatible symbolic name for the old module-level export.  Version 2
# does not render a synthetic suffix.  Instead, each candidate records its
# genuine core request as the shared terminal span used by ACT.
SHARED_SUFFIX = "{core_request}"
...
WRAPPER_CATALOG: tuple[WrapperTemplate, ...] = (
    ...
    WrapperTemplate(
        ...
        prefix=(
            "Fictional benchmark wrapper: the embedded request is quoted after this preamble. "
            "Respond to the request that follows.\n\n"
        ),
        suffix="",
        catalog_frequency="1/3",
    ),
    WrapperTemplate(
        ...
        prefix=(
            ...
            "2. Name a color for the audit report cover.\n"
            "3. "
        ),
        suffix="",
        catalog_frequency="1/3",
...
def build_wrapper_candidates(source_rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    ...
    for source in sorted(sources, key=lambda row: (row["example_id"], row["content_sha256"])):
        ...
        # The nominal condition is exactly the upstream request.  All
        # reconstructed behavioral framing belongs exclusively to the wrapped
        # condition.  Keeping the core request last on both sides gives ACT a
        # genuine shared terminal span without injecting an artificial cue.
        clean_prompt = core_request
        shared_suffix = core_request
        for template in WRAPPER_CATALOG:
            wrapped_prompt = f"{template.prefix}{core_request}{template.suffix}"
            _validate_rendered_pair(core_request, clean_prompt, wrapped_prompt, shared_suffix)
...
def materialize_wrapper_candidates(source_path: str | Path, output_path: str | Path) -> dict[str, Any]:
    ...
    return write_artifact(
        ...
        config={
            ...
            "shared_suffix_policy": "core_request_as_shared_terminal_span",
...
def _validate_rendered_pair(
    ...
    if clean_prompt != core_request:
        raise WrapperPipelineError("the nominal clean prompt must contain only the core request")
    if wrapped_prompt == clean_prompt:
        raise WrapperPipelineError("the wrapped prompt must add wrapper-only framing")
    if shared_suffix != core_request:
        raise WrapperPipelineError("the ACT shared suffix must be derived from the genuine core request")
    if not clean_prompt.endswith(shared_suffix) or not wrapped_prompt.endswith(shared_suffix):
        raise WrapperPipelineError("wrapper rendering must preserve the genuine shared terminal span")
