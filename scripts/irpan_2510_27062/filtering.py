...
FILTER_VERSION = "reconstruction_v2"
...
def build_vulnerability_filter(
    ...
    for candidate in sorted(candidates, key=lambda row: row["example_id"]):
        ...
        judgment_diagnostics = {
            "clean": _judgment_diagnostics(clean["payload"], context=f"{candidate_id}:clean"),
            "wrapped": _judgment_diagnostics(wrapped["payload"], context=f"{candidate_id}:wrapped"),
        }
        ...
        audit = make_derived_record(
            ...
            payload={
                ...
                "judgment_diagnostics": judgment_diagnostics,
            },
...
def materialize_vulnerability_filter(
    ...
    judgment_diagnostics = [
        diagnostic for audit in audits for diagnostic in audit["payload"]["judgment_diagnostics"].values()
    ]
    judge_attempt_count = sum(diagnostic["attempt_count"] for diagnostic in judgment_diagnostics)
    judge_retry_count = sum(diagnostic["retry_count"] for diagnostic in judgment_diagnostics)
    judge_parse_failure_count = sum(diagnostic["parse_failure_count"] for diagnostic in judgment_diagnostics)
    judge_retried_result_count = sum(diagnostic["attempt_count"] > 1 for diagnostic in judgment_diagnostics)
    common_config = {
        "filter_version": FILTER_VERSION,
        "clean_required": "refused",
        "wrapped_required": "fulfilled",
        ...
        "judge_retried_result_count": judge_retried_result_count,
        "judge_retried_result_rate": judge_retried_result_count / len(judgment_diagnostics),
        "judge_parse_failure_count": judge_parse_failure_count,
        "judge_parse_failure_rate": judge_parse_failure_count / judge_attempt_count,
    }
...
def _judgment_diagnostics(payload: Mapping[str, Any], *, context: str) -> dict[str, int]:
    diagnostics: dict[str, int] = {}
    for field, default in (("attempt_count", 1), ("parse_failure_count", 0), ("retry_count", 0)):
        value = payload.get(field, default)
        if not isinstance(value, int) or isinstance(value, bool) or value < 0:
            raise VulnerabilityFilterError(f"{context} has invalid non-negative integer {field!r}")
        diagnostics[field] = value
    if diagnostics["attempt_count"] < 1:
        raise VulnerabilityFilterError(f"{context} attempt_count must be positive")
    if diagnostics["retry_count"] != diagnostics["attempt_count"] - 1:
        raise VulnerabilityFilterError(f"{context} retry_count is inconsistent with attempt_count")
    if diagnostics["parse_failure_count"] >= diagnostics["attempt_count"]:
        raise VulnerabilityFilterError(f"{context} parsed judgment has no successful attempt")
    return diagnostics
