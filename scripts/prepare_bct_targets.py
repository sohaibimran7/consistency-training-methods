...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--progress-dir",
        type=Path,
        help="Resumable per-row checkpoint directory (default: MANIFEST_OUTPUT.progress)",
    )
    parser.add_argument("--max-tokens", type=int, default=32768)
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--max-concurrency", type=int, default=32)
    parser.add_argument(
        "--lora-rank", type=int, default=8, help="Temporary backend LoRA rank; targets use the frozen base"
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.manifest_output.exists():
        parser.error(f"refusing to overwrite completed BCT target artifact: {args.manifest_output}")
    ...
    try:
        progress = BCTProgressStore(progress_dir, progress_identity)
