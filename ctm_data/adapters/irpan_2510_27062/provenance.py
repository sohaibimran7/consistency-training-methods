...
def make_generator_identity(
    *,
    ...
    return {**base, "identity_sha256": sha256_json(base)}
...
def ordered_response_manifest_sha256(entries: Sequence[Mapping[str, Any]]) -> str:
    ...
    return sha256_json(_ordered_response_manifest(entries, expected_example_ids=None))
...
def build_generation_provenance(
    *,
    ...
    payload = {
        ...
        "example_manifest_sha256": sha256_json(examples),
        ...
        "decoding_parameters_sha256": sha256_json(decoding),
        "ordered_response_manifest": responses,
        "ordered_response_manifest_sha256": sha256_json(responses),
...
def validate_generation_provenance(
    ...
    *,
    ...
    computed_example_hash = sha256_json(examples)
    ...
    computed_decoding_hash = sha256_json(decoding)
    ...
    computed_response_hash = sha256_json(responses)
...
def _ordered_response_manifest(
    ...
    *,
    ...
    for index, entry in enumerate(entries):
        ...
        if missing or extra:
            raise ProvenanceError(
                f"ordered_response_manifest[{index}] keys mismatch: missing={missing}, extra={extra}"
            )
    ...
    if expected_example_ids is not None and seen != expected_example_ids:
        ...
        raise ProvenanceError(
            f"response/example manifest identity mismatch: missing={missing_ids}, extra={extra_ids}"
        )
    return normalized
...
def _merge_expected_freshness(expected: Mapping[str, Any] | None, status: str) -> dict[str, Any]:
    ...
    if supplied is not None and supplied != status:
        raise ProvenanceError(
            f"expected target_freshness {supplied!r} conflicts with required status {status!r}"
        )
