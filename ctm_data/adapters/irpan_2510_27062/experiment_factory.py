...
from ctm_data.adapters.irpan_2510_27062.provenance import make_generator_identity
...
_VALIDATION_KEYS = {"jailbreak": {"harmbench", "or_bench"}}
_FINAL_KEYS = {
    "sycophancy": {"mmlu"},
...
def _selected_eval_args(
    ...
    *,
    ...
    if kind != "local_checkpoint":
        raise ValueError(
            f"selected_final_models.{domain}.{method}: trained conditions must use kind: local_checkpoint"
        )
...
def compile_experiment(*, name: str, spec: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for domain in DOMAINS:
        required = {"training_artifact", "bct_result_export", "final"}
        if domain == "jailbreak":
        ...
        data[domain] = {
            "training_artifact": _path(
                section["training_artifact"], label=f"data.{domain}.training_artifact"
            ),
            "bct_result_export": _path(
                section["bct_result_export"], label=f"data.{domain}.bct_result_export"
            ),
            "validation": {key: _path(value, label=f"data.{domain}.validation.{key}") for key, value in validation.items()},
    ...
    examples = {
        domain: _positive_int(examples_raw[domain], label=f"training.examples.{domain}") for domain in DOMAINS
    }
    batch_size = _positive_int(training["batch_size"], label="training.batch_size")
    grad_accum = _positive_int(
        training["gradient_accumulation_steps"], label="training.gradient_accumulation_steps"
    )
    ...
    rmct = {
        "reference_rollouts": _positive_int(
            rmct["reference_rollouts"], label="training.rmct.reference_rollouts"
        ),
        "training_rollouts": _positive_int(
            rmct["training_rollouts"], label="training.rmct.training_rollouts"
        ),
        "consistency_rollouts": _nonnegative_int(
            rmct["consistency_rollouts"], label="training.rmct.consistency_rollouts"
        ),
        "anchor_rollouts": _nonnegative_int(
            rmct["anchor_rollouts"], label="training.rmct.anchor_rollouts"
        ),
        "temperature": _nonnegative_number(
            rmct["temperature"], label="training.rmct.temperature"
        ),
        "max_new_tokens": _positive_int(
            rmct["max_new_tokens"], label="training.rmct.max_new_tokens"
        ),
        "kl_coefficient": _nonnegative_number(
            rmct["kl_coefficient"], label="training.rmct.kl_coefficient"
        ),
        "anchor_weight": _unit_interval(
            rmct["anchor_weight"], label="training.rmct.anchor_weight"
        ),
    }
    ...
    opct = {
        "rollouts_per_prompt": _positive_int(
            opct["rollouts_per_prompt"], label="training.opct.rollouts_per_prompt"
        ),
        "temperature": _nonnegative_number(
            opct["temperature"], label="training.opct.temperature"
        ),
        "max_new_tokens": _positive_int(
            opct["max_new_tokens"], label="training.opct.max_new_tokens"
        ),
        "kl_coefficient": _positive_number(
            opct["kl_coefficient"], label="training.opct.kl_coefficient"
        ),
        "kl_discount_factor": _unit_interval(
            opct["kl_discount_factor"], label="training.opct.kl_discount_factor"
        ),
        "loss": opct["loss"],
    }
        ...
        domain_preparation = [
                {
                    ...
                    "command": [
                        "${python}",
                        "-m",
                        "ctm_data.adapters.irpan_2510_27062",
        ...
        training_jobs.append(
            {
                ...
                "args": {
                    **common,
                    "setting_factory": (
                        "ctm_data.adapters.irpan_2510_27062.rmct_setting:"
                        f"{domain}_rmct_setting"
                    ),
                    "setting_config": {
    ...
    for method in METHODS:
        ...
        model_args = (
            _base_eval_args(model)
            if method == "base" or smoke
            else _trained_eval_args(model, training_name)
        )
        ...
        evaluations.append(
            _eval_entry(
                ...
                target="validation",
                task_factory="ctm_data.adapters.irpan_2510_27062.safety_tasks:paper_safety_tasks",
                task_args={
                    "datasets": [
    ...
    for method in METHODS:
        ...
        for condition, factory in (
            ("clean", "ctm_data.adapters.irpan_2510_27062.mmlu_tasks:mmlu_clean_task"),
            (
                "wrong_suggestion",
                "ctm_data.adapters.irpan_2510_27062.mmlu_tasks:mmlu_wrong_suggestion_task",
            ),
        ):
            evaluations.append(
                _eval_entry(
        ...
        evaluations.append(
            _eval_entry(
                ...
                task_args={
                    "datasets": [
                        data["jailbreak"]["final"][key]
                        for key in sorted(_FINAL_KEYS["jailbreak"])
                    ],
                    "judge_model": judge_model,
                },
    ...
    if smoke:
        ...
        plan["analysis"] = {
            ...
            "args": {
                "input": f"{root}/validation-observations.jsonl",
                "output": f"{root}/selected-candidates.json",
            },
        }
    return plan
