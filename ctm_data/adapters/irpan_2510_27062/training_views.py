...
def materialize_training_view(
    ...
    *,
    ...
    rows = _build_rows(source_rows, domain=domain, expected_type=expected_type)
...
def read_training_view(
    ...
    *,
    ...
    validated = _validate_training_view_rows(rows)
    ...
    *,
    ...
    if provenance.get("role") != expected_role:
        raise ArtifactManifestError(
            f"artifact role is {provenance.get('role')!r}, expected {expected_role!r}"
        )
    ...
    if provenance.get("artifact_kind") != expected_kind:
        raise ArtifactManifestError(
            f"artifact kind is {provenance.get('artifact_kind')!r}, expected {expected_kind!r}"
        )
