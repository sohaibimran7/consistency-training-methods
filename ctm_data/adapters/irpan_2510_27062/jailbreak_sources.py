...
DEFAULT_ID_FIELD = "BehaviorID"
DEFAULT_PROMPT_FIELD = "Behavior"
NORMALIZATION_VERSION = "reconstruction_v1"
...
def normalize_harmbench_rows(
    ...
    *,
    ...
    for index, raw in enumerate(rows, start=1):
        ...
        source_id = raw.get(id_field_value)
        prompt = raw.get(prompt_field_value)
        if not isinstance(source_id, str) or not normalize_text(source_id):
            raise HarmBenchSourceError(f"HarmBench row {index} has missing or non-string {id_field_value!r}")
