...
def build_bct_training_rows(
    ...
    for candidate in retained:
        ...
        generator_sha256 = _sha256(
            target_payload.get("generator_identity_sha256"), field="generator_identity_sha256"
        )
        ...
        rows.append(
            make_derived_record(
                ...
                payload={
                    ...
                    "alignment_strategy": "explicit_core_plus_matching_shared_suffix",
...
def import_stale_targets(
    ...
    *,
    ...
    for candidate in retained:
        ...
        stale_id = (
            f"{candidate_id}:stale_target:{model_sha256[:12]}:{result['target_sha256'][:12]}"
        )
        stale_rows.append(
            make_derived_record(
...
def materialize_training_exports(
    ...
    bct_target_hashes = {
        row["payload"]["candidate_id"]: row["payload"]["target_completion_sha256"] for row in bct_rows
    }
