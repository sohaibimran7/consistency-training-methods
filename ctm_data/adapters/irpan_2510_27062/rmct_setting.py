...
from ctm_data.adapters.irpan_2510_27062.mmlu_tasks import parse_final_answer_label
from ctm_data.adapters.irpan_2510_27062.schema import sha256_json
...
def _read_training_view(
    ...
    *,
    ...
    from ctm_data.adapters.irpan_2510_27062.training_views import read_training_view
...
def _verified_manifest_identity(
    ...
    *,
    ...
    if manifest_domain is not None and manifest_domain != expected_domain:
        raise ValueError(
            f"training view {path} has manifest domain {manifest_domain!r}, expected {expected_domain!r}"
        )
    ...
    if not isinstance(version, int) or isinstance(version, bool):
        raise ValueError(f"training-view manifest for {path} has an invalid schema_version")
...
def _copy_messages(value: Any, *, pair_id: str, field: str) -> list[dict[str, Any]]:
    ...
    for index, message in enumerate(value):
        if not isinstance(message, Mapping):
            raise ValueError(f"pair {pair_id!r} {field}[{index}] must be an object")
        role = message.get("role")
        content = message.get("content")
        if not isinstance(role, str) or not role.strip() or not isinstance(content, str) or not content.strip():
            raise ValueError(
                f"pair {pair_id!r} {field}[{index}] must contain non-empty string role/content fields"
            )
...
def _validated_pair_rows(
    ...
    *,
    ...
    for index, raw in enumerate(rows):
        ...
        row["variant_messages"] = _copy_messages(
            row.get("variant_messages"), pair_id=pair_id, field="variant_messages"
        )
...
def _normalize_label(value: Any, *, field: str) -> str:
    if not isinstance(value, str):
        raise ValueError(f"{field} must be a string option label")
...
def _labels_from_choices(value: Any, *, field: str) -> tuple[str, ...]:
    ...
    if isinstance(value, Mapping):
        ...
        raise ValueError(f"{field} must be a choice mapping or sequence")
    ...
    for location, container in _metadata_containers(datapoint):
        for key in ("choice_labels", "valid_labels"):
            if key in container:
                ...
                labels = tuple(
                    _normalize_label(label, field=f"pair {pair_id!r} {location}.{key}") for label in raw
                )
...
class _PairRMCTSetting:
    ...
    def __init__(self, training_view_path: str | Path | None = None) -> None:
        self.training_view_path = (
            _canonical_path(training_view_path, field="training_view_path")
            if training_view_path is not None
            else None
        )
    ...
    def _resolve_path(self, training_view_path: str | Path | None) -> Path:
        ...
        if self.training_view_path is not None and supplied is not None and supplied != self.training_view_path:
            raise ValueError(
                "exactly one canonical training_view_path is allowed; constructor and load paths disagree"
            )
    ...
    def training_artifact_identity(self) -> dict[str, Any]:
        if self._artifact_identity is None or self._selected_pair_ids is None:
            raise ValueError(
                f"{self.domain} RMCT training selection is unavailable; call load_datapoints first"
            )
    ...
    def __init__(
        ...
        *,
        ...
        if judge is not None and (
            grader_model is not None or grader_config is not None or completion is not None
        ):
        ...
        self.grader_model = (
            injected_model if isinstance(injected_model, str) and injected_model else grader_model or DEFAULT_REFUSAL_MODEL
        )
