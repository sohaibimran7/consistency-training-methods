...
from ctm_data.adapters.irpan_2510_27062.schema import make_source_record
...
def test_partition_registry_has_every_exact_paper_route() -> None:
    ...
    assert sum(len(partitions) for partitions in PARTITION_REGISTRY.values()) == 11
    assert require_partition("mmlu").paper_route == "sycophancy_held_out_reporting"
    assert "not used for model selection" in require_partition("mmlu").notes
