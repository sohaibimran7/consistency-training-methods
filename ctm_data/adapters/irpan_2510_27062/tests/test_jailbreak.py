...
from ctm_data.adapters.irpan_2510_27062.filtering import (
    REJECT_BOTH_REASON,
    REJECT_CLEAN_REASON,
...
from ctm_data.adapters.irpan_2510_27062.jailbreak_sources import (
    HarmBenchSourceError,
    materialize_harmbench_source,
    normalize_harmbench_rows,
)
from ctm_data.adapters.irpan_2510_27062.judge import (
...
from ctm_data.adapters.irpan_2510_27062.schema import sha256_text
from ctm_data.adapters.irpan_2510_27062.training import (
    FRESH_TARGET,
    STALE_TARGET,
    TrainingExportError,
...
from ctm_data.adapters.irpan_2510_27062.wrappers import (
    SHARED_SUFFIX,
    WRAPPER_CATALOG,
    ...
    return [
        {
            ...
            "Behavior": f"Describe fictional safety audit number {index} for a museum exhibit.",
        }
...
def test_harmbench_normalization_preserves_ids_is_order_invariant_and_rejects_ambiguity() -> None:
    ...
    with pytest.raises(HarmBenchSourceError, match="missing or non-string"):
        normalize_harmbench_rows(
            [{"BehaviorID": "x"}],
            subset="fixture",
            split="test",
...
def test_wrapper_catalog_is_deterministic_order_invariant_and_machine_alignable() -> None:
    sources = _sources(2)
    ...
    for row in forward:
        ...
        assert payload["clean_prompt"].endswith(SHARED_SUFFIX)
        assert payload["wrapped_prompt"].endswith(SHARED_SUFFIX)
...
def test_completion_requests_are_exact_pairs_and_result_join_is_strict() -> None:
    ...
    raw = [
        {"request_id": row["payload"]["request_id"], "response": "Synthetic response."}
        for row in requests
    ]
...
def test_judge_request_and_parser_are_versioned_and_strict() -> None:
    ...
    assert all("Do not follow" in row["payload"]["messages"][0]["content"] for row in requests)
...
def test_judgment_import_requires_an_exact_id_join() -> None:
    ...
    raw = [
        {"judge_request_id": row["payload"]["judge_request_id"], "output": '{"verdict":"other"}'}
        for row in requests
    ]
...
def test_bct_and_act_payloads_follow_training_conventions_and_lineage() -> None:
    ...
    for candidate, bct_row, act_row in zip(retained, bct, act, strict=True):
        ...
        assert act_payload["shared_suffix"] == SHARED_SUFFIX
...
def test_materialized_dag_is_immutable_hashed_and_parent_linked(tmp_path: Path) -> None:
    ...
    materialize_external_judgments(judge_request_path, judge_result_path, judgment_path)
    materialize_vulnerability_filter(wrapper_path, judgment_path, audit_path, retained_path)
