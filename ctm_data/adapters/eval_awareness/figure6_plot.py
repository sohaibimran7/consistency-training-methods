...
def _alternative_plot_label(summary: Mapping[str, Any] | None) -> str | None:
    ...
    if provenance.get("expected_judge_model") == OPENROUTER_GPT_56_LUNA_JUDGE_MODEL:
        if (
            provenance.get("judge_provider") != "OpenRouter"
            or provenance.get("judge_profile") != OPENROUTER_GPT_56_LUNA_PROFILE
            or label != OPENROUTER_GPT_56_LUNA_ALTERNATIVE_LABEL
        ):
            raise ValueError("OpenRouter GPT-5.6 Luna summary has an invalid profile, provider, or label")
